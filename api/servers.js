      ...
      if (ram || disk || cpu) {
        ...
        if(newsettings.api.client.webhook.auditlogs.enabled && !newsettings.api.client.webhook.auditlogs.disabled.includes("SERVER")) {
          let params = JSON.stringify({
              embeds: [
                  {
                      title: "Hospedagem Editada",
                      description: `**__Usuário:__** ${req.session.userinfo.username}#${req.session.userinfo.discriminator} (${req.session.userinfo.id})\n\n**__Nova Configuração:__**\n **RAM**: ${checkexist[0].attributes.limits.memory}\n **SSD**: ${checkexist[0].attributes.limits.disk}\n **CPU**: ${checkexist[0].attributes.limits.cpu}%`,
                      color: hexToDecimal("#ffff00")
                  }
              ]
          })
